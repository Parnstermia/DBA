...
public class Agente extends SingleAgent{
    ...
    * @author Diego Alfonso Candelaria Rodríguez y Sergio López Ayala
    */
			...
			if(miBateria.deboRecargar()){
				...
				if ( nodos.isEmpty() ){
					...
					Nodo menor=nodos.get(0);
                                        Nodo novisitado = null;
					for(int i=1 ; i<nodos.size() ; i++){
                                            if(nodos.get(i).distancia < menor.distancia)
                                                    menor=nodos.get(i);
                                            if(!memoria.contains(nodos.get(i).posicion))
                                                    novisitado = nodos.get(i);
                                            System.out.println("\nEL NODO MENOR ES: "+menor);
					}
                                        
					if(!memoria.contains(menor.posicion))
					     orden=menor.direccion.getString();
                                        else if (novisitado != null)
                                             orden =novisitado.direccion.getString();
                                        else{
                                            int randomNum = ThreadLocalRandom.current().nextInt(0, nodos.size());
                                             orden=nodos.get(randomNum).direccion.getString();
                                        }
